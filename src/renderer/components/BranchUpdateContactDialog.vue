<template>
	 <v-dialog v-model="contactdialog" max-width="500px">
       <v-btn slot="activator" icon class="mx-0" @click="contactDetail(item)">
        ...
        <form @submit.prevent="personSaveChanges">
        <v-card>
          <v-card-title>
...
export default {
	...
	methods: {
    isThereInMobileNo () {
      if (this.contactDetails.MOBNO.trim()) {
      const checkInduplicatePerson = (personsRepository.existInmobile(this.contactDetails.MOBNO.trim()))
      this.errorMessages = checkInduplicatePerson && checkInduplicatePerson.BRCOD !== this.contactDetails.BRCOD ? 'Already exists' : [] 
      }
      return true
      },
		contactDetail (item) {
    ...
    personSaveChanges () {
      this.$store.dispatch('personSavingChanges', this.contactDetails)
